...
def vanilla_vector_product(a: Vector, b: Vector) -> float:
    """
...
def vanilla_solution(x: Vector, y: Vector, A: Matrix, B: Matrix) -> Vector:
    ...
    """
    def vector_add(a: Vector, b: Vector) -> float:
        result = []
        for ele_a, ele_b in zip(a, b):
            result.append(ele_a + ele_b)
        return result

    return vector_add(vector_add(vanilla_matmul(A, x), vanilla_matmul(vanilla_transpose(A), x)), vanilla_matmul(vanilla_transpose(B), y))
...
def numpy_solution(
    ...
    """
    return A.dot(x) + A.T.dot(x) + B.T.dot(y)
    raise NotImplementedError("Your Code Goes Here")
...
def main():
    ...
    """
    RNG = np.random.RandomState(seed=675)
